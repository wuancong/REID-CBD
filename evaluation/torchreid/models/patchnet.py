...
import torch
from torchreid.models.resnet import MetaLinear,MetaConv2d,MetaModule,MetaBatchNorm2d
import torchreid
...
class PatchGenerator(nn.Module):
    def __init__(self):
        ...
        self.localization = nn.Sequential(
            MetaConv2d(2048, 4096, kernel_size=3),
            MetaBatchNorm2d(4096),
            nn.ReLU(True),
...
class PatchNet(nn.Module):
    def __init__(self, class_num=1000, backbone=resnet50, pretrained=True, param_path='../../imagenet_models/resnet50-19c8e357.pth',
        ...
        self.baseline = torchreid.models.build_model(
                        name='resnet50_fc512',
        ...
        self.stripe = 6
        down = nn.ModuleList([nn.Sequential(MetaConv2d(2048, 256, 1),
                                            MetaBatchNorm2d(256),
        ...
        if self.is_for_test is False:
            fc_list = nn.ModuleList([MetaLinear(256, class_num) for _ in range(self.stripe)])
            self.new.add_module('fc_list', fc_list)
            embedding = nn.ModuleList([MetaLinear(256, 128) for _ in range(self.stripe)])
